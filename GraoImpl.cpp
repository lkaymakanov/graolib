...
jstring getPersonInfo(JNIEnv *env,  jstring egn,  std::wstring(*strFun)(PROPERTYNAME_VALUE *) ){
	...
	/*
	char *lpBuff = (char*)env->GetStringUTFChars(egn, 0);
	_strupr(lpBuff);
	jstring jstr = env->NewStringUTF(lpBuff);
	env->ReleaseStringUTFChars(egn, lpBuff);
	return jstr;*/
...
JNIEXPORT void JNICALL printPersonInfo(WCHAR *idn){
	...
	std::wstring json = createJson(propNameValue);
	free(str);
...
*/
HRESULT getPersonInfo(BSTR ein, 
...
*/
std::wstring createJson(PROPERTYNAME_VALUE *arraypNameValue){
	std::wstring res(L"");
	int i =0;
	...
	for(; i < PROP_CNT; i++){
		...
		larray[cnt++] = &arraypNameValue[i];  //store element pointer in local array!!!
	}
	i=0;
	...
	for(; i < cnt-1; i++){
		...
		res+=(quote + propName + quote + colon + quote + propValue + quote + comma + nL);
	}
	//add the last element to json string
	std::wstring propName(larray[i]->propName.propNameWideChar);
	std::wstring propValue(larray[i]->propValue == NULL ? L"null" : larray[i]->propValue);
	res+=(quote + propName + quote + colon + quote + propValue + quote + nL );
	res= bjson + res + ejson;
